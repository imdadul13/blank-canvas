    ...
    >
      <div className="relative bg-white/88 backdrop-blur-2xl rounded-3xl max-w-xl w-full shadow-[inset_0_1px_1.5px_rgba(255,255,255,0.98),0_25px_60px_rgba(0,107,99,0.12)] border border-white/85 overflow-hidden flex flex-col max-h-[calc(100dvh-max(2.5rem,calc(1.5rem+env(safe-area-inset-top,0px)+env(safe-area-inset-bottom,0px))))] sm:max-h-[90vh] animate-in zoom-in-95 duration-150">
        ...
        <div className="px-3 sm:px-6 pt-2.5 pb-2.5 sm:pt-3 sm:pb-3 border-b border-stone-200/80 bg-[#FAF9F5] flex items-center gap-1.5 overflow-x-auto scrollbar-none">
          {[
            { id: 'theme', label: 'Themes & Visuals', shortLabel: 'Themes', icon: Palette },
            { id: 'mcq', label: 'Pacing & Goals', shortLabel: 'Pacing', icon: Clock },
            { id: 'wellness', label: 'Cognitive Breaks', shortLabel: 'Wellness', icon: Droplet },
            { id: 'storage', label: 'Storage & Reset', shortLabel: 'Storage', icon: HardDrive },
          ].map((tab) => {
            const Icon = tab.icon;
            ...
            return (
              <button
                ...
                className={`inline-flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3.5 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all cursor-pointer ${
                  active
        ...
        {/* Tab Body */}
        <div className="p-5 sm:p-6 pb-8 sm:pb-6 overflow-y-auto flex-1 space-y-6 bg-white">
        ...
        {/* Footer */}
        <div
          className="p-4 bg-[#FAF9F5] border-t border-stone-200/80 flex items-center justify-between"
