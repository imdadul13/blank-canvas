...
export interface MoreUtilityItem {
  ...
  desc: string;
  tab?: ActiveTab;
  action?: 'cloudsync' | 'settings' | 'profile';
}
...
export const SidebarDock: React.FC<NavbarProps> = ({
  ...
  const isSecondaryActive = activeTab === 'grandtests' || activeTab === 'telegram' || activeTab === 'more';
...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <>
      ...
      <nav
        ref={mobileMoreRef}
        className="lg:hidden fixed left-1/2 -translate-x-1/2 z-50 max-w-[calc(100vw-1rem)] w-auto bg-white/95 backdrop-blur-xl border border-slate-200/80 shadow-[0_8px_30px_rgba(0,0,0,0.12)] rounded-full px-1.5 py-1 font-['Plus_Jakarta_Sans']"
      ...
      >
        <div className="flex items-center gap-0.5 sm:gap-1">
          {mobileNavItems.map(({ id, label, icon: Icon }) => {
            ...
            return (
              <button
                ...
                type="button"
                onClick={() => setActiveTab(id)}
                className={`flex items-center justify-center gap-1 h-9 px-2 sm:px-2.5 rounded-full transition-all duration-150 cursor-pointer ${
                  active
                    ? 'bg-[#006B63] text-white shadow-xs font-semibold px-2.5 sm:px-3'
          ...
          <button
            ...
            title="More"
          >
            <MoreHorizontal className={`h-4 w-4 ${mobileMoreOpen || isSecondaryActive ? 'text-white stroke-[2.2]' : 'text-slate-500 stroke-[1.8]'}`} />
            {(mobileMoreOpen || isSecondaryActive) && (
              <span className="text-[11px] font-semibold tracking-tight whitespace-nowrap">More</span>
            )}
          </button>
        </div>
        ...
        {mobileMoreOpen && (
          <div
            className="absolute bottom-[calc(100%+0.5rem)] right-0 left-0 sm:left-auto sm:w-72 z-50 rounded-2xl bg-white/95 backdrop-blur-xl border border-slate-200/80 shadow-[0_12px_40px_rgba(0,0,0,0.16)] p-2 font-['Plus_Jakarta_Sans'] animate-in fade-in zoom-in-95 duration-150"
            role="menu"
              ...
              return (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    {desc && <span className="block text-[10px] text-slate-400 truncate">{desc}</span>}
                  </span>
                </button>
              );
            })}
